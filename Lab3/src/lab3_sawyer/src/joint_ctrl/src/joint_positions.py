...
def map_keyboard(side):
    ...
    while not done and not rospy.is_shutdown():
        joint_angles = input('Input 7 joint angles here: ')
        joint_angles_list = joint_angles.split()

        joint_angles_dict = {}
        for i,value in enumerate(joint_angles_list):
            value = float(value)
            joint_angles_dict[str(joints[i])] = value

        time = rospy.get_time()
        while rospy.get_time() - time <= 5:
            limb.set_joint_positions(joint_angles_dict)

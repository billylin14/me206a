...
def skew_3d(omega):
    ...
    # YOUR CODE HERE
    omega_hat = np.zeros((3,3), dtype=type(omega[0]))
...
def rotation_3d(omega, theta):
    ...
    R = i_3+omega_hat/norm*np.sin(norm*theta)+np.matmul(omega_hat, omega_hat)/norm**2*(1-np.cos(norm*theta))
    return R
...
def hat_3d(xi):
    ...
    # YOUR CODE HERE
    w_hat = skew_3d(xi[3:])
    xi_hat = np.append(w_hat, xi[:3].reshape((3,1)), axis=1)
    xi_hat = np.append(xi_hat, [np.zeros(4)], axis=0)
    return xi_hat
...
def homog_3d(xi, theta):
    ...
    # new = (1/norm**2)*(np.matmul(np.eye(3)-exp, np.matmul(w_hat,v))+np.matmul(np.outer(w, w.T), v)*theta)
    # M = np.append(exp, new.reshape((3,1)), axis=1)
...
def prod_exp(xi, theta):
    ...
    # YOUR CODE HERE
    g = np.eye(4)
    for x, th in zip(xi.T, theta.T):
        g = np.matmul(g, homog_3d(x.T, th))
    return g

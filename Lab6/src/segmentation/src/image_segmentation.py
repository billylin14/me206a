...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    return np.logical_and(gray_img > lower_thresh, gray_img < upper_thresh)*1
...
def edge_detect_naive(gray_img):
    ...
    blur = np.pad(blur, pad_width=1, mode='constant', constant_values=0)
...
def cluster_segment(img, n_clusters, random_state=0):
    ...
    height, width, channel = img_d.shape
    img_r = img_d.reshape(height*width, channel)
...
def segment_image(img): 
    ...
    #binary = threshold_segment_naive(to_grayscale(img), 50, 80).astype(np.uint8)
...
if __name__ == '__main__':
    # adjust the file namestest_cluster here

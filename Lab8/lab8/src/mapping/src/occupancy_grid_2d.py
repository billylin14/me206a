...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            # TODO!
            angle_world = yaw + (msg.angle_min + msg.angle_increment*idx)
            ...
            if r > msg.range_max:
                ...
                continue
            if r < msg.range_min:
                rospy.logwarn("mapping_node.py%s: Range %f < %f was too small.",
            ...
            # TODO!
            n = min(self._x_res, self._y_res)
            prev_grid_x = np.nan
            prev_grid_y = np.nan
            for i, length in enumerate(np.arange(r, 0, -n)):
                world_x = sensor_x + length*np.cos(angle_world)
                world_y = sensor_y + length*np.sin(angle_world)
                grid_x, grid_y = self.PointToVoxel(world_x, world_y)
                
                if not (grid_x == prev_grid_x and grid_y == prev_grid_y):
                    if i == 0:
                        self._map[grid_x, grid_y] = min(self._occupied_update + self._map[grid_x,grid_y], self._occupied_threshold)
                    else:
                        self._map[grid_x, grid_y] = max(self._free_update + self._map[grid_x,grid_y], self._free_threshold)

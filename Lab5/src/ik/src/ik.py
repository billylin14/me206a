...
def ik_service_client():
    ...
    gripper_config = input('Enter x, y, and z coordinates for end effector: ').split()
    gripper_config = [float(i) for i in gripper_config]

    # Set end effector position: YOUR CODE HERE
    pose_stamped.pose.position.x = gripper_config[0]
    pose_stamped.pose.position.y = gripper_config[1]
    pose_stamped.pose.position.z = gripper_config[2]
    
    # Set end effector quaternion: YOUR CODE HERE
    pose_stamped.pose.orientation.x = float(0)
    pose_stamped.pose.orientation.y = float(1)
    pose_stamped.pose.orientation.z = float(0)
    pose_stamped.pose.orientation.w = float(0)

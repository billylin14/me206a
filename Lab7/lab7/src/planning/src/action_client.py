...
def main(argv):
    #Extract start and goal angles
    angle_start = [float(i) for i in argv[:4]]
    angle_goal = [float(i) for i in argv[4:]]
    print(angle_start, angle_goal)
    #Initialize the node
    ...
    goal.request.start_state.joint_state.position = [angle_start[0], angle_start[1], angle_start[2], angle_start[3], 0.0, 0.0, 0.0]
    ...
    target_joint_angles = [angle_goal[0], angle_goal[1], angle_goal[2], angle_goal[3], 0.0, 0.0, 0.0]
    tolerance = 0.0001
    consts = []
